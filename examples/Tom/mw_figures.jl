...
fig = Figure(resolution = (1920, 1080))
    ...
    ax2 = Axis(fig[3, 1], xticks=(inspection_dates[I_char2], [L"t_{\nu_1^{(2)}, \phi_{1, 1}^{(2)}}", L"t_{\nu_1^{(2)}, \phi_{1, 2}^{(2)}}", L"t_{\nu_2^{(2)}, \phi_{2, 1}^{(2)}}", L"t_{\nu_2^{(2)}, \phi_{2, 2}^{(2)}}"]), ylabel = L"\textrm{Characteristic } (2): ARD_\infty", ylabelsize=lbsize2, xgridvisible=false, xticklabelsize=lbsize)
    linkxaxes!(ax1, ax2)
    linkyaxes!(ax1, ax2)
...
fig = Figure(resolution = (1920, 1080))
    ...
    ax1 = Axis(fig[1, 1],
        ...
        ylabel = L"\textrm{Characteristic } (1): ARD_1", ylabelsize=lbsize2)
    axτ1 = Axis(fig[1, 1], 
        xaxisposition=:top, xlabel="Maintenance Date", xticks=(τ, [L"\tau_{%$i}" for i in eachindex(τ)]), xgridvisible=false, xticklabelsize=lbsize, xlabelsize=lbsize2,
        yticks=([], []), yticklabelsvisible=false, ygridvisible=false)
    linkxaxes!(ax1, axτ1)
    ...
    ax2 = Axis(fig[3, 1],
        xticks=([], []), xgridvisible=false,
        ylabel = L"\textrm{Characteristic } (2): ARD_\infty", ylabelsize=lbsize2)
    linkxaxes!(ax1, ax2)
    linkyaxes!(ax1, ax2)
