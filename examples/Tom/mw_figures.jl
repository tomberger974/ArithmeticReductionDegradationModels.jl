...
# Maintenance dates and types
τ = T .* [.17, .4, .5, .8]
maint_types = unique(key[2] for key in keys(ρ))
...
for i in eachindex(μ_list)
    fig = Figure(resolution = (1920, 1080))
    ax = Axis(fig[1, 1], xgridvisible=false, xlabelsize=lbsize2)
...
fig = Figure(resolution = (1920, 1080))
    ...
    text!(ax1, inspection_dates[nearest_time(τ[4], inspection_dates) - 30], (Y1[nearest_time(τ[4], inspection_dates) - 1] + Y1[nearest_time(τ[4], inspection_dates)]) / 2 - 1.5, text=L"- \rho_2 x", color=:blue, fontsize=lbsize)
    ...
    text!(ax2, inspection_dates[nearest_time(τ[4], inspection_dates) - 30], (Y∞[nearest_time(τ[4], inspection_dates) - 1] + Y∞[nearest_time(τ[4], inspection_dates)]) / 2 - 1, text=L"- \rho_2 x", color=:red, fontsize=lbsize)
...
fig = Figure(resolution = (1920, 1080))
    ...
    # virtual increments for the sole last observations of the second characteristic
    ps=Point2f.([(t, 0) for t in inspection_dates[I][end-2:end]])
    text_virtual_var = [L"\Delta X_{5, 2}^{(1)}", L"\Delta X_{5, 3}^{(1)}"]
    for i in 1:length(ps)-1
        arrows2d!(ax1, ps[i], ps[i+1],
        argmode = :endpoint,
        tail = Point2f[(0, 0), (1, -0.5), (1, 0.5)], taillength = 8
